...
export default function ContributionHistory() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Contribution History</h1>
      ...
      <div className="grid sm:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="text-3xl font-bold text-slate-900">{displayContributions.length}</div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
          ...
          {displayContributions.map((contribution, index) => {
            ...
            return (
              <div key={contribution.id} className="relative flex gap-6">
                {/* Timeline node */}
                <div className={`relative z-10 flex-shrink-0 w-12 h-12 rounded-full ${getColor(contribution.type)} flex items-center justify-center`}>
                ...
                <div className="flex-1 pb-6">
                  <div className="bg-slate-50 rounded-lg p-5">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium ${getColor(contribution.type)} mb-2`}>
                          {contribution.type.charAt(0).toUpperCase() + contribution.type.slice(1)}
                        </span>
                        <h3 className="font-semibold text-slate-900">{contribution.description}</h3>
                      </div>
                    </div>

                    {contribution.impact && (
                      <p className="text-sm text-slate-600 mb-3">
                        <span className="font-medium text-emerald-600">Impact:</span> {contribution.impact}
                      </p>
                    )}

                    <p className="text-xs text-slate-500">{formatDate(contribution.created_at)}</p>
                  </div>
                </div>
              </div>
            );
        ...
        {displayContributions.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-slate-900 mb-2">No contributions yet</h3>
